...
const QRGeneratorPage = () => {
    ...
    const idFromLocalStorage = JSON.parse(localStorage.getItem("login-mhs"))

    const nimFromLocalStorage = idFromLocalStorage.nim
    {console.log(idFromLocalStorage.id)}
    console.log(nimFromLocalStorage)

    const date = new Date()
    const thisDay = date.getDate() + "-" + (date.getMonth() + 1) + "-" + date.getFullYear()

    useEffect(() => {
        fetch(`https://64f2052d0e1e60602d24967d.mockapi.io/students/${idFromLocalStorage.id}`)
